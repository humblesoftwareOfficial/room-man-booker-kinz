...
import { EReservationDuration, formatDateToYYYYMMDD, formatInterValDate } from "../../utils/dates";
...
const CustomDatePickerRange = ({ onClose, onBook, labelButton = "Enregistrer" }) => {
  ...
  const renderDuration = useCallback(
    () =>
      Object.keys(EReservationDuration).map((value, _index) => (
        ...
        >
          <Text style={{ fontSize: 10 }}>{`${EReservationDuration[value]}`}</Text>
        </TouchableOpacity>
      )),
    [duration]
  );
  ...
  return (
    <View style={{ flex: 1}}>
      <View style={styles.header}>
        <Text style={styles.title}>Date de réservation</Text>
