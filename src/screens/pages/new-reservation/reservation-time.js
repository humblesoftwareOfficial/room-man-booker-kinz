...
import { convertDateStringFormat, isValidHour, isValidMinute } from "../../../utils/dates";
...
const ReservationTime = ({ onNext, onBack }) => {
  ...
  return (
    <View style={styles.container}>
      <View style={NEW_ROOM_STYLE.header}>
        <Text style={NEW_ROOM_STYLE.title}>Heures de réservation</Text>
      </View>
      {renderStartTime()}
      {renderEndTime()}
      <CustomButton
        ...
        onClick={onNext}
        disable={
          !isFieldWithValue(newReservation?.timeStartHour) ||
          !isFieldWithValue(newReservation?.timeStartMinute) ||
          !isFieldWithValue(newReservation?.timeEndHour) ||
          !isFieldWithValue(newReservation?.timeEndMinute) ||
          !isValidHour(newReservation?.timeStartHour) ||
          !isValidMinute(newReservation?.timeStartMinute) ||
